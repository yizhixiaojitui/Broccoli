...
function changeImage() {
	$('#verifyCodeImage').attr(
			'src',
			basePath + '/Login/getVerifyCode.action?timestamp='
					+ genTimestamp());

}
function changeImage2() {
	$('#rverifyCodeImage').attr(
			'src',
			basePath + '/Login/getVerifyCode.action?timestamp='
					+ genTimestamp() + '1');
...
function login() {
	...
	$.ajax({
		type : 'post',
		url : basePath + "/Login/toLogin.action",
		data : {
			username : username,
			password : password,
		...
		success : function(data) {
			...
			if (msg != null || msg != "" && msg == 0) {
				...
				window.location.href=basePath + "/home/index.action";
			}
...
function LoginTestUser() {
	$('#username').blur(function() {
		...
		$.ajax({
			type : 'post',
			url : basePath + "/Login/loginTestUser.action",
			data : {
				username : name
			},
			dataType : "json",
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (msg == 0) {
				...
				window.setTimeout("window.location='" + basePath
						+ "/home/index.action'", 2000);
...
function testUser() {
	$('#rusername').blur(function() {
		...
		$.ajax({
			type : 'post',
			url : basePath + "/Register/testUser.action",
			data : {
				username : name
			},
			// dataType : "json",
	...
	$('#remail')
			.blur(
					function() {
						...
						$.ajax({
							type : 'post',
							url : basePath + "/Register/testEmail.action",
							data : {
								email : email
							},
							// dataType : "json",
...
function testNike() {
	$('#nikename').blur(function() {
		...
		$.ajax({
			type : 'post',
			url : basePath + "/Register/testID.action",
			data : {
				nikename : nikename
			},
			// dataType : "json",
